...
public class RedisRedPacketServiceImpl implements RedisRedPacketService {
     ...
     */
    @Override
    //开启新线程运行,让Spring自动创建另外一条线程去运行它,这样它便不在抢最后一个红包的线程之内
    @Async
        ...
        for(int i = 0;i < times; i++){
            ...
            for (int j = 0; j < userIdList.size() ; j++) {
                ...
                userRedPacket.setRedPacketId(redPacketId)
                        ...
                        .setGrabTime(new Timestamp(time))
                        .setNote("抢红包 " + redPacketId);
                userRedPacketList.add(userRedPacket);
            }
            //插入抢红包信息
            count += executeBath(userRedPacketList);
        }
        //删除redis列表
        redisTemplate.delete(PREFIX + redPacketId);
        Long end = System.currentTimeMillis();
        System.err.println("保存数据结束,耗时 " + (end - start) +
                "毫秒,共" + count + "条记录被保存");
    }
     ...
     *
     */
    private int executeBath(List<UserRedPacket> userRedPacketList){
